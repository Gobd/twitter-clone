...
var tweetIdMaker = function() {
  return Math.floor(Math.random()*1000000);
};

var newTweet = function(tweetText, tweetTime, tweetId) {
  var tweetIdString = tweetId.toString();
  idList.push(tweetIdString);
  localStorage.setItem('tweetList', idList);
  localStorage.setItem(tweetId, JSON.stringify({tweetText: tweetText, tweetTime: tweetTime, tweetId: tweetId}));
  $('#char-count').text('140');
  return ('<div class="tweet" id="' + tweetId + '"><div class="content"><img class="avatar" src="img/alagoon.jpg" /><strong class="fullname">BKing</strong><span class="username">@tasty</span><p class="tweet-text">' + tweetText + '</p><div class="tweet-actions"><ul><li><span class="icon action-reply"></span> Reply</li><li><span class="icon action-retweet"></span> Retweet</li><li><span class="icon action-favorite"></span> Favorite</li><li><span class="icon action-more"></span> More</li></ul></div><div class="time">' + '<p><span data-livestamp="' + tweetTime + '"></span></p>' + '</div></div>');
  };
  ...
  $('.button').on('click', function() {
    ...
    if ($('textarea.tweet-compose').val()) {
      //prepend new tweet
      $('#stream').prepend(newTweet($('textarea.tweet-compose').val(), date.toISOString(), tweetIdMaker()));
    }
